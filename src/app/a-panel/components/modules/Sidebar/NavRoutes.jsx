...
function NavRoutes({ onLinkClick }) {
  ...
  const navRoutes = useMemo(() => {
    const details = notifications?.details || [];
    ...
    return [
      ...
      },
      {
        ...
        icon: FaRegComments,
        statusNumber: (details[0]?.count || 0) + (details[1]?.count || 0),
      },
      {
        href: '/a-panel/message',
        label: 'مدیریت پیام ها',
        icon: BiMessageSquare,
        statusNumber: details[5]?.count || 0,
      },
      {
        href: '/a-panel/tickets',
        label: ' مدیریت تیکت ها',
        icon: MdMailOutline,
        statusNumber: (details[2]?.count || 0) + (details[3]?.count || 0),
      },
      {
        href: '/a-panel/questions',
        label: ' مدیریت سوال ها',
        icon: FaRegCircleQuestion,
        statusNumber: details[4]?.count || 0,
      },
      {
        ...
        >
          ...
          >
            <div className='flex items-center gap-1'>
              <route.icon className='text-xl sm:text-2xl' />
            ...
            {route.statusNumber > 0 && (
              <span className='rounded-full bg-red px-2 py-0.5 font-faNa text-sm text-white'>
                {route.statusNumber}
              </span>
            )}
            {route.subRoutes && (
