  ...
  const submitReply = async () => {
    ...
    try {
      ...
      const replyData = {
        ...
        parentId: comment.id,
        ...(isCourse
          ? { courseId: comment.courseId } // مقدار دهی courseId
          : { articleId: comment.articleId }), // مقدار دهی articleId
      };
      ...
      // بررسی وضعیت پاسخ
      if (!response.ok) {
        throw new Error('ثبت پاسخ نا موفق بود!');
      }
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm'>
      <div className='relative max-h-screen w-11/12 overflow-y-auto rounded-xl bg-surface-light p-6 xs:w-5/6 sm:w-2/3 dark:bg-background-dark'>
        ...
        <div className='mt-6 flex flex-wrap items-center gap-2'>
          <Image
            src={
              comment?.avatar ? comment.avatar : '/images/default-profile.png'
            }
