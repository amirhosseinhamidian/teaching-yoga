...
const SeoGeneralForm = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`/api/admin/seo/general`);
  ...
  const validateInputs = () => {
    ...
    if (!description.trim()) {
      errors.description = 'توضیحات متا نمی‌تواند خالی باشد.';
    }
    if (!openGraphTitle.trim()) {
      errors.openGraphTitle = 'عنوان اپن گراف نمی‌تواند خالی باشد.';
    }
    if (!openGraphDescription.trim()) {
      errors.openGraphDescription = 'توضیحات اپن گراف نمی‌تواند خالی باشد.';
    }
    if (!openGraphImageAlt.trim()) {
      errors.openGraphImageAlt = 'متن تصویر اپن گراف نمی‌تواند خالی باشد.';
    }
    if (!openGraphImageUrl.trim()) {
      errors.openGraphImageUrl = 'تصویری برای اپن گراف انتخاب کنید.';
    }

    if (!openGraphUrl.trim()) {
      errors.openGraphUrl = 'آدرس سایت برای اپن گراف نمی تواند خالی باشد.';
    }

    const urlPattern =
      /^(https?:\/\/)?([\w-]+\.)+[\w-]{2,4}(:\d+)?(\/[\w\-\\.]+)*\/?(\?[;&a-z\\[\]=\\+\-_]*\w+)?(#\w+)?$/i;
    if (!urlPattern.test(openGraphUrl)) {
      errors.openGraphUrl = 'آدرس سایت برای اپن گراف را به درستی وارد کنید.';
    }

    if (!urlPattern.test(canonicalTag)) {
  ...
  const handleAddTag = () => {
    ...
    if (tagList.find((tagElem) => tagElem === tag)) {
      setTag('');
      return;
    }
  ...
  const handleImageChange = async (event) => {
    ...
    if (file) {
      ...
      formData.append('fileName', 'general');
  ...
  return (
    <div className='pb-56'>
      <div className='flex flex-wrap items-center justify-between'>
        <PageTitle>سئو {'>'} تنظیمات کلی</PageTitle>
        <Button
      ...
      <div className='mt-6 grid grid-cols-1 sm:grid-cols-2'>
        <div className='flex gap-2'>
          <Input
            ...
            onChange={setTitle}
            label='عنوان سایت'
            placeholder='عنوانی که در تب مرورگر و نتایج جستجو نمایش داده می‌شود'
            maxLength={60}
            isShowCounter
          ...
          />
          <Tippy
            content='عنوانی که در تب مرورگر و نتایج جستجو نمایش داده می‌شود. این عنوان به عنوان شناسایی سایت شما در مرورگرها و موتورهای جستجو به نمایش درمی‌آید.'
            animation='fade'
          >
      ...
      <div className='mt-8 flex gap-2'>
        ...
        />
        <Tippy
          content='توضیحات متا برای سئو سایت بسیار مهم است و باید به طور مختصر و جذاب محتوای کلی سایت شما را توضیح دهد. این توضیحات در نتایج جستجو نمایش داده می‌شود و به بهبود رتبه سایت کمک می‌کند.'
          animation='fade'
        >
          <span className='self-end'>
            <FiInfo
      ...
      <div className='mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2'>
        <div className='flex gap-2 self-start'>
          ...
          <Tippy
            content='کلمات کلیدی به موتورهای جستجو کمک می‌کنند تا محتوای سایت شما را بهتر شناسایی کنند. این کلمات باید مرتبط با محتوای سایت و موضوعات اصلی شما باشند. استفاده از کلمات کلیدی به بهبود رتبه سایت در نتایج جستجو کمک می‌کند. برای استفاده مؤثرتر، از کلمات کوتاه و دقیق استفاده کنید و مطمئن شوید که تعداد آن‌ها بیش از حد نباشد.'
            animation='fade'
          >
            <span className='self-end'>
              <FiInfo
      ...
      <div className='mt-8 flex gap-2'>
        ...
        />
        <Tippy
          content='متنی است که هنگام اشتراک‌گذاری لینک سایت شما در شبکه‌های اجتماعی نمایش داده می‌شود. این متن باید خلاصه، جذاب و مرتبط با محتوای صفحه باشد تا کاربران را برای کلیک و بازدید جذب کند.'
          animation='fade'
        >
          <span className='self-end'>
            <FiInfo
      ...
      <div className='mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2'>
        <div className='flex gap-2'>
          <Input
            ...
            label='آدرس سایت برای اپن گراف'
            placeholder='آدرس صفحه اول سایت را وارد کنید'
            className='bg-surface-light text-xs sm:text-sm dark:bg-surface-dark'
          />
          <Tippy
            content='URL سایت در تنظیمات Open Graph باید آدرس کامل و معتبر صفحه اصلی وب‌سایت شما باشد. این آدرس باید شامل "http://" یا "https://" باشد و به‌درستی کاربران را به سایت هدایت کند.'
            animation='fade'
          >
            <span className='self-end'>
              <FiInfo
